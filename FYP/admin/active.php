	...
	<div class="col-sm-2">
	 ...
	 <!-- End Of header-->
			  ...
			  <tbody>
						...
						<?php
	$active= $class->fetchdata("SELECT * FROM `post` WHERE status='active' && user_id='$ok' ");		
		while($activepost=$active->fetch(PDO::FETCH_ASSOC))
		{
			extract($activepost);
			?>
							<tr>
				   <td><img src="../images/<?php echo $img1;?>" style="width:80px;height:80px;"/></td>
					<td><?php echo $title ?></td>
				  <td><?php echo $price ?></td>
				  
				  <td><a href="detail.php?id=<?php echo $activepost['id']; ?>&status='active'" type="button" class="btn btn-success">Detail</a> <a href="edit.php?id=<?php echo $activepost['id']; ?>&status='active'" type="button" class="btn btn-success">Edit</a>  </td>
					</tr>
