...
?>	 <!--header-->
	...
	<div class="col-sm-2">
	 ...
	 <!-- End Of header-->
		...
		while($detail=$post->fetch(PDO::FETCH_ASSOC))
		{
			extract($detail);
			?>
							<tr>
			...
			$userName=$user->fetch(PDO::FETCH_ASSOC);
				echo $userName['name'];  
				  ...
				  {
				?>
	<a href="poststatus.php?from=mypost&id=<?php echo $id; ?>&status=Pending" type="button" class="btn btn-success">Active</a> 
				<?php
				  }else{				  
				  ?>
	<a href="poststatus.php?from=mypost&id=<?php echo $id; ?>&status=Active" type="button" class="btn btn-danger">Pending</a> 
				  <?php
				  }
				  ?>
