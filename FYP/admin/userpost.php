...
$pending= $class->fetchdata("select * from post where status='active' and user_id!='$ok'");
		...
		while($detail=$pending->fetch(PDO::FETCH_ASSOC))
		{
			...
			$userName=$user->fetch(PDO::FETCH_ASSOC);
				echo $userName['name'];  
				  ...
				  ?></td>
				  <td>
	<a href="poststatus.php?id=<?php echo $id; ?>&status=Pending" type="button" class="btn btn-success"><?php echo $status;?></a> 
